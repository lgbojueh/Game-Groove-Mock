...
export default function ResultsPage() {
  ...
  // Extract the full query string once per render
  const queryString = searchParams.toString();

  useEffect(() => {
    // Derive filter arrays inside the effect
    ...
    // Load cached searchResults
    const stored = localStorage.getItem("searchResults");
    let results: BasicGame[] = stored ? JSON.parse(stored) : [];

    // Helper to apply a multi-select filter
    const applyFilter = (selected: string[], prop: keyof BasicGame, list: BasicGame[]) =>
      selected.length === 0 ? list : list.filter(g => g[prop] != null && selected.includes(g[prop]!));
    ...
    setGames(cleaned);
    setLoading(false);
  }, [queryString, searchParams]); // only re-run when the actual query string or searchParams changes
